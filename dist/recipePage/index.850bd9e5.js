...
parcelHelpers.export(exports, "goBack", ()=>goBack);
// Go to bookmark page - handler
parcelHelpers.export(exports, "bookmarksBtnHandler", ()=>bookmarksBtnHandler);
// Bookmarking functionality - handler
parcelHelpers.export(exports, "bookmarkPageHandler", ()=>bookmarkPageHandler);
...
(function init() {
    ...
    window.addEventListener("load", async ()=>{
        const data = await (0, _modelJsDefault.default).loadRecipes();
        (0, _viewJsDefault.default).renderMeals(data);
    });
    getRecipeId(".recipe__container");
    getRecipeId(".bookmark__list");
    (0, _viewJsDefault.default).takeSearchInput();
    bookmarksBtnHandler();
})();
// Checks for existing bookmarks on start
(function renderExistingBookmarks() {
    const data = (0, _modelJsDefault.default).checkExistingBookmarks();
    if (data) data.forEach((meal)=>{
        (0, _viewJsDefault.default).renderBookmark(meal.image, meal.name, `${meal.area} Meal`, meal.id);
    });
})();
...
(async function checkForHash() {
    try {
        switch(location.hash){
            case "#bookmarks":
                (0, _viewJsDefault.default).showBookmarks();
                break;
            default:
                const data = await (0, _modelJsDefault.default).clickRecipe(location.hash.slice(1));
                (0, _viewJsDefault.default).renderEntireRecipe(data);
                (0, _viewJsDefault.default).checkMainPageVisibility("#back-btn", "none", "block").checkMainPageVisibility("#input_recipe", "block", "none");
                break;
        }
    } catch (err) {
        (0, _helpersJs.throwErr)(err);
    }
})();
// Load more recipes functionality
(function loadMoreRecipes() {
    document.querySelector(".load__more").addEventListener("click", async function() {
        (0, _modelJsDefault.default).startNum += 20;
        (0, _modelJsDefault.default).endNum += 20;
        const data = await (0, _modelJsDefault.default).loadRecipes();
        (0, _viewJsDefault.default).renderMeals(data);
    });
})();
function goBack(el) {
    document.querySelector(el).addEventListener("click", function() {
        location.hash = "";
        (0, _viewJsDefault.default).makeHidden(".all__bookmarks").closeRecipePage();
        document.querySelector(".recipe__container").style.display = "grid";
        (0, _viewJsDefault.default).checkMainPageVisibility(el, "none", "block").checkMainPageVisibility("#input_recipe", "block", "none");
    });
}
goBack("#back-btn");
////////////////////////////////////////////////////////
...
function getRecipeId(mainEl) {
    document.querySelector(mainEl).addEventListener("click", async (e)=>{
        ...
        if (el) {
            (0, _viewJsDefault.default).id = el?.dataset?.id;
            const data = await (0, _modelJsDefault.default).clickRecipe((0, _viewJsDefault.default).id);
            (0, _viewJsDefault.default).renderEntireRecipe(data);
            (0, _viewJsDefault.default).checkMainPageVisibility("#back-btn", "none", "block").checkMainPageVisibility("#input_recipe", "block", "none");
            location.hash = (0, _viewJsDefault.default).id;
        }
    });
}
function bookmarksBtnHandler() {
    document.querySelector(".bookmarks").addEventListener("click", function() {
        (0, _viewJsDefault.default).showBookmarks().checkMainPageVisibility("#back-btn", "none", "block").checkMainPageVisibility("#input_recipe", "block", "none");
    });
}
function bookmarkPageHandler() {
    document.querySelector(".wrap__recipe-page").addEventListener("click", async function(e) {
        try {
            const btn = e.target.closest(".bookmark__button");
            if (!btn) return;
            const data = await (0, _modelJsDefault.default).bookmarkRecipe(location.hash.slice(1));
            if (data[1]) return;
            data[0].meals.forEach((meal)=>{
                (0, _viewJsDefault.default).renderBookmark(meal.strMealThumb, meal.strMeal, `${meal.strArea} Meal`, meal.idMeal);
            });
        } catch (err) {
            console.error(err);
        }
...
},{"./model.js":"Py0LO","./view.js":"2GA9o","./helpers.js":"6s1be","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","core-js/modules/web.immediate.js":"49tUX"}],"Py0LO":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _webImmediateJs = require("core-js/modules/web.immediate.js");
var _helpersJs = require("./helpers.js");
class Model {
    ...
    async loadRecipes() {
        try {
            const res = await (0, _helpersJs.API_URL)("search.php?s=");
            const data = await res.json();
            if (this.endNum >= data.meals.length) (0, _helpersJs.showInfo)("Reached The End!");
            return data.meals.slice(this.startNum, this.endNum);
        // return data.meals;
        } catch (err) {
            (0, _helpersJs.throwErr)(err.message);
        }
    }
    // Clicking on recipe and fetching data functionality
    async clickRecipe(id) {
        try {
            const res = await (0, _helpersJs.API_URL)(`lookup.php?i=${id ? id : ""}`);
            const data = await res.json();
    ...
    async searchForRecipe(input) {
        try {
            const res = await (0, _helpersJs.API_URL)(`search.php?s=${input}`);
            const data = await res.json();
            // If meals return a falsy value
            if (!data.meals) (0, _helpersJs.showInfo)("No meals found!");
            return data;
        } catch (err) {
            (0, _helpersJs.throwErr)(err);
        }
    ...
    async bookmarkRecipe(id) {
        try {
            const res = await (0, _helpersJs.API_URL)(`lookup.php?i=${id}`);
            const data = await res.json();
            if (!data.meals) throw new Error("Something went unsuccessfully, please try again!");
            let checkForId;
            data.meals.forEach((meal)=>{
                checkForId = this.#bookmarks.some((bookmark)=>meal.idMeal === bookmark.id);
                if (!checkForId) this.#bookmarks.push({
                    name: meal.strMeal,
    ...
    checkExistingBookmarks() {
        const data = localStorage.getItem("bookmarks") ? JSON.parse(localStorage.getItem("bookmarks")) : "";
        if (!data) return;
        data.forEach((meal)=>{
            this.#bookmarks.push({
                name: meal.name,
...
},{"./helpers.js":"6s1be","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","core-js/modules/web.immediate.js":"49tUX"}],"6s1be":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "API_URL", ()=>API_URL);
parcelHelpers.export(exports, "throwErr", ()=>throwErr);
parcelHelpers.export(exports, "showInfo", ()=>showInfo);
var _webImmediateJs = require("core-js/modules/web.immediate.js");
var _configJs = require("./config.js");
var _viewJs = require("./view.js");
var _viewJsDefault = parcelHelpers.interopDefault(_viewJs);
const API_URL = async (url)=>{
    (0, _viewJsDefault.default).showSpinner(document.querySelector(".recipe__container"));
    const api = await fetch(`https://themealdb.com/api/json/v2/${(0, _configJs.API_KEY)}/${url}`);
    (0, _viewJsDefault.default).hideSpinner();
    if (!api.ok) throw new Error("Something went wrong.. Please try again.");
    return api;
};
const throwErr = (err)=>(0, _viewJsDefault.default).showErrorMsg(err);
const showInfo = (info)=>(0, _viewJsDefault.default).showInfoMsg(info);

},{"./config.js":"4Wc5b","./view.js":"2GA9o","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","core-js/modules/web.immediate.js":"49tUX"}],"4Wc5b":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class InterfaceControl {
    ...
    showInfoMsg(infoMsg) {
        ...
        this.#hideMsg = setTimeout(()=>infoMsgEl.style.transform = "translateY(100px)", 3000);
    }
    // Check if main page is visible and acts accordingly
    checkMainPageVisibility(el, display1, display2) {
        !this.checkIfHidden(document.querySelector(".recipe__container")) ? document.querySelector(el).style.display = display1 : document.querySelector(el).style.display = display2;
        return this;
    }
    showSpinner() {
    ...
    render(img, title, desc, id) {
        document.querySelector(".recipes__wrapper").insertAdjacentHTML("beforeend", this.#markup(img, title, desc, id));
        return this;
    }
    openRecipePage(img, title, instruction) {
        const ingredientUl = document.querySelector(".ingredients");
        this.clearHTML(ingredientUl);
        document.querySelector(".recipe__img").style.backgroundImage = `url('${img}')`;
        document.querySelector(".recipe__title").textContent = title;
        document.querySelector(".recipe__instructions-text").textContent = instruction;
        this.hideMainPage().makeHidden(".all__bookmarks");
        this.#recipePage.style.display = "flex";
        _controllerJs.bookmarkPageHandler();
    ...
    insertIng(ing) {
        const ingredientUl = document.querySelector(".ingredients");
        ingredientUl.insertAdjacentHTML("beforeend", `<li><span>${ing}</span></li>`);
    }
    renderEntireRecipe(data) {
        if (data) {
            this.openRecipePage(data?.strMealThumb, data?.strMeal, data?.strInstructions);
            // Looping over the recipe keys and values and uses it to render out all the data
            for (const [key, value] of Object.entries(data))if (key.startsWith("strIngredient")) value && this.insertIng(value);
        }
    }
    closeRecipePage() {
    ...
    _expandImg() {
        ...
        this.#recipePage.addEventListener("click", function(e) {
            ...
            expandedImg.firstElementChild.src = getComputedStyle(clickedEl).getPropertyValue("background-image").slice(5).slice(0, -2);
            correctThis.#isImgOpen = !correctThis.#isImgOpen;
            !correctThis.#isImgOpen ? expandedImg.style.scale = 0 : expandedImg.style.scale = 1;
        });
    }
    takeSearchInput() {
        const correctThis = this;
        ...
        searchInput.addEventListener("input", async function() {
            const data = await (0, _modelJsDefault.default).searchForRecipe(this.value);
            correctThis.clearHTML(document.querySelector(".recipes__wrapper")).makeHidden(".load__more");
    ...
    `;
        document.querySelector(".bookmark__list").insertAdjacentHTML("afterbegin", markup);
    }
}
exports.default = new InterfaceControl();

},{"./model.js":"Py0LO","./controller.js":"1GgH0","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","core-js/modules/web.immediate.js":"49tUX"}],"49tUX":[function(require,module,exports) {
...
},{}]},["1Mgbh","1GgH0"], "1GgH0", "parcelRequiref3c5")
